...
export async function authenticate(
  ...
  const resolvedAnchor =
    typeof anchor === 'string'
      ? {
          homeDomain: anchor,
          WEB_AUTH_ENDPOINT: await getWebAuthEndpoint(anchor),
          capabilities: { sep10: true },
        }
      : anchor;

  const cached = getCachedJwt(resolvedAnchor.homeDomain, publicKey);
  if (cached) return cached;

  const webAuthEndpoint = resolvedAnchor.WEB_AUTH_ENDPOINT;
  if (!webAuthEndpoint || !resolvedAnchor.capabilities.sep10) {
    throw new Error(
      `Anchor "${resolvedAnchor.homeDomain}" does not support SEP-10 authentication.`
    );
  }
  ...
  const auth: Sep10Auth = { jwt, anchorDomain: resolvedAnchor.homeDomain, publicKey, expiresAt };
  setCachedJwt(auth);
  return auth;
}

export const signSep10Challenge = signChallenge;
export const exchangeSep10Token = submitChallenge;
