...
function isUserRejectedError(error: unknown): boolean {
  ...
  return (
    e.code === -4 || /user rejected|user declined|declined|denied|cancelled|canceled/i.test(msg)
  );
}
...
function hasSignatureFromPublicKey(
  ...
  const hash = tx.hash();
  return tx.signatures.some((sig) => signer.verify(hash, sig.signature()));
}
...
// ─── Challenge fetch (legacy — no homeDomain, no typed error) ─────────────────
