...
// SEPs that indicate routable transfer capability for end-user off-ramps
// (SEP-6 programmatic transfer, SEP-24 interactive transfer).
// SEP-31 (cross-border direct payment) requires bilateral sending-anchor
// agreements; SEP-31-only anchors are tracked for health but never routed.
const ROUTABLE_SEPS: ReadonlyArray<NonNullable<Anchor['seps']>[number]> = ['sep6', 'sep24'];

/**
 * Returns true if the anchor supports at least one routable transfer SEP
 * (SEP-6 or SEP-24). Issuer-only anchors and SEP-31-only anchors are excluded
 * from corridor selectors and the rate engine.
 */
...
/**
 * Returns true if the anchor supports SEP-31 and lacks both SEP-6 and SEP-24.
 * Such anchors are tracked in the registry for health monitoring but are never
 * returned for end-user off-ramp routing.
 */
...
/**
 * Returns all registered anchors in full, including SEP-31-only and issuer-only
 * anchors. Health probes and administrative monitors use this; end-user routing
 * must never use this directly.
 */
