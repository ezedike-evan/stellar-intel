...
type LegacyWithdrawRequest = Sep24WithdrawRequest & {
  transferServer: string;
};
...
export function computeRateComparison(
  ...
  const best = rates.reduce((a, b) => {
    const aTotal = a.totalReceived ?? -Infinity;
    const bTotal = b.totalReceived ?? -Infinity;

    if (bTotal !== aTotal) {
      return bTotal > aTotal ? b : a;
    }

    return b.anchorId.localeCompare(a.anchorId) < 0 ? b : a;
  });
...
export async function initiateWithdraw(
  anchorOrParams: ResolvedAnchor | LegacyWithdrawRequest,
  params?: Sep24WithdrawRequest,
  signal?: AbortSignal
): Promise<Sep24WithdrawResponse> {
  const legacyParams = params === undefined;
  const request = legacyParams ? (anchorOrParams as LegacyWithdrawRequest) : params!;
  const anchor = legacyParams ? null : (anchorOrParams as ResolvedAnchor);
  const transferServer = legacyParams
    ? (anchorOrParams as LegacyWithdrawRequest).transferServer
    : anchor?.TRANSFER_SERVER_SEP0024;
  const anchorName = anchor?.homeDomain ?? transferServer;
  const { jwt, assetCode, assetIssuer, amount, account } = request;

  if (!transferServer || (anchor && !anchor.capabilities.sep24)) {
    throw new Error(`Anchor "${anchorName}" does not support SEP-24 withdrawals.`);
  }
