...
function findCoverageGaps(covered: ReadonlySet<string>, startDay: string, endDay: string): CoverageGap[] {
...
export function buildProbeCoverageReport(
  ...
  const anchorRows: AnchorProbeCoverage[] = anchors.map(({ anchorId, domain }) => {
    ...
    const streak =
      continuousStreakEndingAt(covered, asOfDay) ??
      continuousStreakEndingAt(covered, lastProbeDay);
