...
1. **Every leg is signed by the user.** Value moves only on a transaction the user
   ...
   a held key. Intents may carry a user signature (optional attestation); value moves
   only on a transaction the user signs in their wallet ([`docs/INTENT_API.md`](INTENT_API.md)).
...
1. **The reputation oracle** (see [`docs/ORACLE_SPEC.md`](ORACLE_SPEC.md))
   ...
   delivered rate, settle time, and outcome status. Today, the oracle records
   per-anchor outcomes with no user identifier (`OutcomeLogRow` stores no user key,
   and PII is redacted per [`docs/NON_CUSTODY.md`](NON_CUSTODY.md)). A credit layer
   would need a way to correlate or prove a user's own remittance history — whether
   through client-side attestation or zero-knowledge proof — that is user-signed,
...
1. Should synthetic probe transactions (nightly $1 off-ramps — see Rung 1
   in [`docs/ROADMAP.md`](ROADMAP.md#rung-1--data-infra)) count toward proven
   history? Probes are user-witnessed but may not reflect genuine remittance
   behaviour.
