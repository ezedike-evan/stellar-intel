...
> `anchor.seps?.some(sep => ROUTABLE_SEPS.includes(sep)) ?? false` where `ROUTABLE_SEPS = ['sep6', 'sep24']`.
> An entry with no `seps` — or with only `sep10` / `sep38` / `sep31` — is excluded from
> `getAnchorsByCorridorId`, so the anchor never reaches a corridor selector or
> the rate engine. SEP-31-only anchors are tracked for health but never routed, and
> must use `corridors: []` plus `sep31Corridors`. Declare at least one of `sep6` or `sep24`
> for routable off-ramps.
