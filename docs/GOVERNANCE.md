...
- Rust toolchain + `cargo` with the `stellar` CLI installed.
...
The contract upgrade authority (bound at deploy by
`__constructor(admin, upgrade_admin)`; `upgrade` in
`contracts/reputation/src/upgrade.rs`) uses a separate `UpgradeAdmin` key. The
same multisig migration path applies: rotate `UpgradeAdmin` to the multisig
account using `propose_upgrade_admin`, `accept_upgrade_admin`, and
