...
| **MITM on anchor traffic**       | Tampered `stellar.toml` or quote in transit                         | TLS to anchor domains; anchor calls run server-side; SEP-10 pins the network passphrase and requires an https `WEB_AUTH_ENDPOINT`.                                                                                     |
| **Hostile SEP-10 challenge**     | Anchor serves a payment or other transaction as the login challenge | `WebAuth.readChallengeTx` against the toml `SIGNING_KEY` before signing: sequence 0, `manage_data`-only ops for this wallet and home domain, current timebounds, server signature. Missing `SIGNING_KEY` fails closed. |
