...
These terms may change. Material changes will be reflected in the "Last reviewed"
...
`components/offramp/DisclaimerBanner.tsx` renders `DISCLAIMER_TEXT` from
`lib/legal.ts`, which mirrors this block, and links here, so this document is
the single source for the wording.
