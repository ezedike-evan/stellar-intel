...
export type ProbeFailureType = 'dns' | 'tls' | 'http' | 'timeout' | 'mismatch' | 'integrity' | 'unknown';
...
function mapProbeRowRecord(r: Record<string, unknown>): ProbeLedgerRow {
  ...
  const reachable = reachableRaw === 1 || reachableRaw === true || reachableRaw === 'true' || reachableRaw === 't';
...
export function aggregateProbeSignals(
  ...
  const signals: ProbeSignals[] = domains.map((domain) => {
    ...
    const uptimeRatio =
      uptimeRows.length === 0 ? null : uptimeRows.filter((r) => r.reachable).length / uptimeRows.length;
...
interface OracleSubmitClient {
   ...
   */
  publish_probe_signals?: (args: {
    publisher: string;
    payload: string;
  }) => AssembledTx;

  /** Legacy alias some local deploys expose. */
  set_probe_signals?: (args: {
    publisher: string;
    payload: string;
  }) => AssembledTx;
}
...
export async function publishProbeSignals(
  ...
  if (typeof publishFn !== 'function') {
    ...
    console.log('[publisher] probe signals skipped — contract has no publish_probe_signals entrypoint');
    return 0;
  }
  ...
  try {
    const assembled = await withRetry(
      async () => {
        const tx = await (publishFn as (a: { publisher: string; payload: string }) => AssembledTx).call(
          client,
          {
